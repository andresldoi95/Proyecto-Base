...
public class CargarDatosNuevaCosecha extends AsyncTask<Void, Void, Void> {
    ...
    protected void onPostExecute(Void aVoid) {
        ...
        activity.spnFormatoEntrega.setAdapter(new ArrayAdapter<>(activity, android.R.layout.simple_spinner_dropdown_item, formatosEntrega));
        activity.spnOrigenMadera.setAdapter(new ArrayAdapter<>(activity, android.R.layout.simple_spinner_dropdown_item, origenesMadera));
        activity.spnOrigenHacieda.setAdapter(new ArrayAdapter<>(activity, android.R.layout.simple_spinner_dropdown_item, origenesHacienda));
        ...
        if (tipoMadera != null && origenHacienda != null) {
            ...
            if(currentCosecha != null){
                for (int i = 0; i < origenesHacienda.size(); i++) {
                    if (origenesHacienda.get(i).id == currentCosecha.origenHaciendaId) {
                        activity.spnOrigenHacieda.setSelection(i);
                        break;
                    }
                }
            }
