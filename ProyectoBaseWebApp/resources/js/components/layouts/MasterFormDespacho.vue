  ...
  <section>
    <form @submit.prevent="submit">
      <b-field v-show="tipo_formulario === 'C'" grouped group-multiline>
        ...
        <b-select v-show="camiones_filter.length > 0" @input="submit" v-model="form.camion_filter_id">
          <option value="">Seleccione un Camión</option>
          <option
            v-for="option in camiones_filter"
            :key="option.id"
            :value="option.id"
          >{{ option.placa }}</option>
        </b-select>
        <b-select v-show="destinos_filter.length > 0" @input="submit" v-model="form.destino_filter_id">
          <option value="">Seleccione un Destino</option>
          <option
            v-for="option in destinos_filter"
            :key="option.id"
            :value="option.id"
          >{{ option.descripcion }}</option>
        </b-select>
        <b-select v-show="origenes_madera_filter.length > 0" @input="submit" v-model="form.origen_madera_filter_id">
          <option value="">Seleccione una Hacienda</option>
          <option
            v-for="option in origenes_madera_filter"
            :key="option.id"
            :value="option.id"
          >{{ option.descripcion }}</option>
        </b-select>
...
export default {
  ...
  mounted: function () {
    this.cargarCamionesFilter();
    this.cargarDestinoFilter();
    this.cargarOrigenesMaderaFilter();
    this.submit();
  },
};
</script>
